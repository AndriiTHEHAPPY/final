...
# Приховуємо технічні попередження
...
pygame.display.set_caption("Кото-Паркур, Космобій та Змійка")
...
vorona_img = get_img("vorona", (80, 80))  # Твоя ворона замість будинку
...
# --- СТВОРЕННЯ КНОПОК З УКРАЇНСЬКИМ ТЕКСТОМ ---
btn_snake = Button(290, 150, 220, 60, "Змійка", "images/button_snake.jpg")
btn_shooter = Button(290, 230, 220, 60, "Космобій", "images/button_spacebattle.png")
btn_dino = Button(290, 310, 220, 60, "Кото-Паркур", "images/button_catparkour.jfif")
# Кнопка налаштувань у лівому куті
btn_settings = Button(20, 20, 180, 50, "Інфо", "images/settings.png")
...
def draw_score(val, health=None):
    txt = score_font.render(f"Рахунок: {val}", True, WHITE)
    screen.blit(txt, (WIDTH - 180, 20))
    if health is not None:
        htxt = score_font.render(f"Життя: {health}", True, RED)
        screen.blit(htxt, (WIDTH - 180, 60))


# --- ЕКРАН ІНСТРУКЦІЙ ---
def run_settings():
    screen.blit(menu_bg, (0, 0))
    overlay = pygame.Surface((600, 420), pygame.SRCALPHA)
    overlay.fill((0, 0, 0, 200))
    screen.blit(overlay, (100, 90))

    instructions = [
        "КЕРУВАННЯ В ІГРАХ:",
        "",
        "ЗМІЙКА: Стрілочки (Рух)",
        "КОСМОБІЙ: Стрілки - кермо, Пробіл - вогонь",
        "КОТО-ПАРКУР: Пробіл - Стрибок (подвійний)",
        "",
        "ОБ'ЄКТИ:",
        "Миша - 1 бал | Вірус - шкода",
        "Аптечка - лікує корабель",
        "",
        "Натисніть ESC для виходу в меню"
    ]
    for i, line in enumerate(instructions):
        color = YELLOW if "КЕРУВАННЯ" in line else WHITE
        txt = score_font.render(line, True, color)
        screen.blit(txt, (140, 130 + i * 35))
...
def run_dino():
    ...
    roof_x -= speed;
    roof_dist += speed
    if roof_dist >= WIDTH: score += 1; roof_dist = 0
    ...
    if random.random() < 0.015:
        type_obj = random.choice(["pipe", "vorona"])
        img, y_p = (pipe_img, 410) if type_obj == "pipe" else (vorona_img, 390)
...
def run_shooter():
    ...
    for r in repair_items[:]:
        ...
        if r.colliderect(ship_rect):
            ...
            if sounds.repair_snd: sounds.repair_snd.play()
...
def run_snake():
    ...
    if now - last_snake_update > 150:
        ...
        if head == apple_pos:
            if sounds.mouse_eat_snd: sounds.mouse_eat_snd.play()  # ПИЩИТЬ ТУТ!
            score += 1;
            apple_pos = [random.randrange(0, WIDTH // S_SIZE) * S_SIZE, random.randrange(0, HEIGHT // S_SIZE) * S_SIZE]
            if random.random() < 0.3:
                virus_pos = [random.randrange(0, WIDTH // S_SIZE) * S_SIZE,
                             random.randrange(0, HEIGHT // S_SIZE) * S_SIZE]
        else:
            snake_pos.pop()
        if head == virus_pos:
            if sounds.virus_snd: sounds.virus_snd.play()
            score = max(0, score - 1)
            for _ in range(2):
    ...
    for i, p in enumerate(snake_pos):
        if i == 0:
            screen.blit(
                pygame.transform.rotate(s_head_img, {"UP": 0, "DOWN": 180, "LEFT": 90, "RIGHT": 270}[snake_dir]), p)
        else:
            screen.blit(s_body_img, p)
    draw_score(score)
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE and state != MENU:
            pygame.mixer.music.stop();
            state = MENU
